...
export function LiveDashboard() {
  ...
  const currentRacer = current?.isPit
    ? (next ??
      computed
        ...
        .find((c) => !c.isPit))
    : current;
  const nextAction = running && idx >= 0 ? computed[idx + 1] : !running ? computed[0] : undefined;
  ...
  const belowMinStint = !!current && !current.isPit && stintElapsedMin < state.config.minStint;
  ...
  const pitMinRemainingMs = current?.isPit
    ? Math.max(
        0,
        current.startAt +
    ...
    <div className="space-y-4">
            ...
            <div className="flex gap-2">
              {current?.isPit ? (
                ...
                >
                  ...
                  {belowMinStint ? `Box em ${fmtCountdown(minStintRemainingMs)}` : "Box"}
                </Button>
              )}
                ...
                <div className="relative overflow-hidden rounded-xl border-l-4 border-success bg-success/10 p-4">
                  <div className="absolute right-3 top-3">
                    {running && current && (
                      <Badge
                        variant="default"
                        className="gap-1 border-success/50 bg-success uppercase tracking-wider text-success-foreground hover:bg-success"
                      >
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Stat
          ...
          value={
            next ? (next.suggestedBallast > 0 ? `${next.suggestedBallast} kg` : "Sem lastro") : "—"
          }
          hint={
            next
        ...
        <Stat
          ...
          tone={running && completedStops >= summary.requiredStops ? "success" : "default"}
        />
        <Stat
          ...
          info={
            <div className="space-y-1">
              ...
              <p>
                ...
                {running
                  ? fmtDuration(Math.max(0, (pitCloseAt - now) / MIN))
                  : fmtDuration(closeOffset)}
              </p>
              <p>
      ...
      <div className="panel p-4">
        ...
        <ul className="space-y-2">
          {computed.slice(running ? idx + 1 : 0, (running ? idx + 1 : 0) + 6).map((c, i) => (
            <li
              key={c.id}
              className={`flex items-center justify-between rounded-md px-3 py-2 ${
                i === 0 ? "border border-primary/30 bg-primary/5" : "bg-secondary/60"
              }`}
            >
          ...
          {timelineOpen && (
            <div className="border-t border-border px-4 py-4">
              <p className="mb-3 text-xs text-muted-foreground">
                {burnable} turno(s) ao mínimo de {state.config.minStint} min e {remainingStops}{" "}
                paragens de {minPit} min. Estimativa{" "}
                {running ? "a partir de agora" : "a partir da partida"}.
              </p>
      ...
      <AlertDialog open={confirmStop} onOpenChange={setConfirmStop}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              A cronometragem é interrompida e o plano volta ao estado anterior à partida. Esta ação
